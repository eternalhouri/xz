        ...
        <div class="row pr-3">
          <div id="preview" class="col pr-0">
            <div class="card bg-transparent mr-5 position-absolute">
              <img class="card-img-top" :src="mImgSrc" alt="Card image cap">
              <div id="mask" :style="maskStyle" class="position-absolute" v-show="show"></div>
              <div id="super-mask" class="position-absolute" @mouseover="display"
              @mouseout="hide"
              @mousemove="move($event)"></div>
              <div id="div-lg" class="position-absolute" :style="lgDivStyle" v-show="show"></div>
              <div class="card-body p-0 text-center">
                <img src="img/product_detail/hover-prev.png" class="btn float-left btn-light border-0 p-1 pt-4 pb-4" :class="btnLeftDisabled==true?'disabled':''" @click="moveRight">
                <div class="d-inline-block pt-2 mx-0 m-auto">
                  <!-- <ul class="list-unstyled mb-0"> -->
                  <ul :style="ulImgsStyle" class="list-unstyled mb-0">
                    <li v-for="(p,i) of pics" :key="i" class="float-left p-1">
                      <img :src="p.sm" @mouseenter="changeImg(i)">
                    </li>
                ...
                <img src="img/product_detail/hover-next.png" class="btn float-right btn-light border-0 p-1 pt-4 pb-4" :class="btnRightDisabled==true?'disabled':''" @click="moveLeft">
              </div>
            </div>
          </div>
          <div id="details" class="col pl-0">
            ...
            <div>
              ...
              <div class="float-left w-75">
                <router-link v-for="(sp,i) of specs" class="btn btn-sm btn-outline-secondary" 
                :class="lid==sp.lid?'active':''" :key="i" :to="`/details/${sp.lid}`" v-text="sp.spec"></router-link>
                <!--删除后三个a-->
              </div>
...
<script>
  //1. 引入header.vue组件对象，并起名
  import myHeader from "@/components/header.vue"
  export default {
//{path:"/details/:lid",...,props:true}
    props:["lid"],//接url中的lid参数
    data(){
      return {
        maskStyle:{//支持mask的位置移动
          left:0,
          top:0
        },
        mImgSrc:"",//专门支持mImg的src属性
        lgDivStyle:{//专门支持lgDiv的背景图片和背景定位
          "background-image":"",
          "background-position":"0px 0px"
        },
        show:false,//控制mask和lgdiv的显示和隐藏
        ulImgsStyle:{//专门控制ulImgs的样式
        //因为ulImgs在整个效果中会有两个样式发生变化，所以必须有两个属性来支持变化
          width:0,
          "margin-left":0
        },
        moved:0,//记录ul已经左移的li的次数
        //lid:,
        product:{price:0},//防止.toFixed()出错
        specs:[],
        pics:[]
      }
    },
    ...
    methods:{
      move(e){
        ...
        this.lgDivStyle["background-position"]=`${-top*16/7}px ${-left*16/7}px`;
      },
      display(){
        this.show=true;
      },
      ...
      changeImg(i){//v-for=" i of pics"
        //i:0 1 2 3
        //取出pics中i位置的一个图片对象
        var pic=this.pics[i];
        //分别取出当前图片的md和lg，分别给mImg和lgDiv
        this.mImgSrc=pic.md;
      ...
      load(){
        ...
        this.axios.get(
          ...
          //根据pics中的图片张数，初始化ulImgs的宽度
          this.ulImgsStyle.width=
            this.pics.length*62+"px"
          //获得pics中第一张图片初始化mImg和lgDiv的图片
    ...
    //2. 设置myHeader组件对象为当前组件的子组件
    components:{
      myHeader //Vue会自动翻译为<my-header>
    }
  }
</script>
